...
class StackBiRNN_char(StackBiRNN):
    def __init__(self, *args, **kwargs):
        super(StackBiRNN_char, self).__init__(*args)

    def char(self):
            return self.s[-1][2]

class StackRNN_char(StackRNN):
    def __init__(self, *args, **kwargs):
        
        super(StackRNN_char, self).__init__(*args)
    
    def char(self):
        return self.s[-1][1]

class DeleteRNN_char(StackRNN_char):
    def clear_all(self):
        self.s = self.s[:1]
...
class SamplableTransitionInflector(TransitionInflector):
    ...
    def transduce(self, lemma, feats, form=None, oracle_actions=None, sampling=True, external_cg=False):
        def _valid_actions_test(buffer):
            valid_actions = []
            if len(buffer) > 0:
        ...
        def _valid_actions(buffer,stack, word, form):

            valid_actions = []
            if form is not None:
#                if len(buffer) > 0:
#                    print buffer.char()[1], stack.char(), u''.join(word), u''.join(form)
#                else:
#                    print "None", stack.char(), u''.join(word), u''.join(form)
                if len(word)<len(form):
                    insert_char = form[len(word)]
                    if len(buffer) > 0: #there is smth to generate from or copy or delete
                        if buffer.char()[1] == insert_char: #stack is the same as the next char
                            valid_actions += [self.COPY]
                        else:
                            valid_actions += [self.DELETE]
                            valid_actions += [self.vocab_acts.w2i[insert_char]]
#                        valid_actions += [self.DELETE, self.vocab_acts.w2i[insert_char]]
                    else:
                        valid_actions += [self.vocab_acts.w2i[insert_char]]
                else:
                    valid_actions += [self.STOP]
            else:
                valid_actions = _valid_actions_test(buffer)
            return valid_actions
        ...
        stack  = StackRNN_char(self.stackRNN, self.pempty_stack_emb)
        buffer = StackBiRNN_char(self.fbuffRNN, self.bbuffRNN, self.pempty_buffer_emb)
        delete = DeleteRNN_char(self.deleteRNN, self.pempty_delete_emb)  # has method to empty stack
        action_stack = StackRNN_char(self.actRNN, self.pempty_act_emb)
...
#            valid_actions = _valid_actions(stack, buffer)
            valid_actions = _valid_actions(buffer, stack, word, form)
            ...
            prob_probs = dy.softmax(logits)
#            print len(prob_probs.value())
#            print prob_probs.value()
            log_probs = dy.log_softmax(logits, valid_actions)
#            print len(log_probs.value())
#            print log_probs.value()
...
#            losses.append(dy.pick(log_probs, action))
            losses.append(dy.pick(dy.log(prob_probs), action))
...
if __name__ == "__main__":
    ...
    print 'Loading data...'
    train_data = FullActionDataSet.from_file(train_path)
    dev_data = FullActionDataSet.from_file(dev_path)
    if test_path:
        test_data = FullActionDataSet.from_file(test_path)
    else:
        test_data = None
...
#                        reward += 0.5 * sum(c == COPY_CHAR for c in predicted_actions)/ len(predicted_actions)
#                        reward = 40. if prediction == word else -1                                # reward for accuracy
#                        reward -= 0.1 * len(predicted_actions)/len(word) # the less actions per lemma the better
                        rewards_indiv =  [2 / len(word) if c == COPY_CHAR else 1/ len(word) for c in predicted_actions]
                        reward = 1 if not sum(c == DELETE_CHAR for c in predicted_actions) == len(lemma) else  -0.01
                        
#                        print len(rewards_indiv), len(loss)
                        loss_indiv = dy.dot_product(dy.inputVector(rewards_indiv), dy.concatenate(loss))
#                        if prediction == word:
...
#                        batch_loss.append(dy.average(loss))
                        batch_loss.append(-loss_indiv)
                        rewards.append(reward)
                    i += 1
                    if i < 5:
                        print prediction, word, predicted_actions, reward, rewards_indiv, [float(np.exp(l.npvalue())) for l in loss]
