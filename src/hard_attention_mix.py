...
Usage:
  hard_attention.py [--dynet-mem=MEM] [--dynet-seed=SEED] [--input=INPUT] [--hidden=HIDDEN]
...
#        alphabet.append(marker)
    ...
    if not eval_only:
        ...
        train_lemmas_padded = [BEGIN_WORD + w + END_WORD for w in train_lemmas]
        train_words_padded = [BEGIN_WORD + w + END_WORD for w in train_words]
        train_word_pairs_padded = zip(train_lemmas_padded,train_words_padded)
...
def train_model_wrapper(input_dim, hidden_dim, layers, train_lemmas, train_feat_dicts,
    ...
    # build model
    if reload:
        initial_model, char_lookup, feat_lookup, R, bias, encoder_frnn, encoder_rrnn, decoder_rnn, W_p_gen, bias_p_gen = load_best_model(alphabet, results_file_path, input_dim, hidden_dim, layers, feature_alphabet, feat_input_dim, feature_types)
    else:
        initial_model, char_lookup, feat_lookup, R, bias, encoder_frnn, encoder_rrnn, decoder_rnn, W_p_gen, bias_p_gen = build_model(alphabet, input_dim, hidden_dim, layers, feature_types, feat_input_dim, feature_alphabet)
    ...
    predicted_sequences = predict_sequences(trained_model, char_lookup, feat_lookup, R, bias, encoder_frnn, encoder_rrnn, decoder_rnn, alphabet_index,
                                            ...
                                            feat_index,
                                            feature_types, W_p_gen, bias_p_gen, beam, nbest=0)
...
def build_model(alphabet, input_dim, hidden_dim, layers, feature_types, feat_input_dim, feature_alphabet):
    ...
    concatenated_input_dim_p_gen = 2 * hidden_dim + hidden_dim + input_dim + len(feature_types) * feat_input_dim
    W_p_gen = model.add_parameters((1, concatenated_input_dim_p_gen))
    bias_p_gen = model.add_parameters(1)
    
    print 'decoder lstm dimensions are {} x {}'.format(concatenated_input_dim, hidden_dim)
    print 'finished creating model'

    return model, char_lookup, feat_lookup, R, bias, encoder_frnn, encoder_rrnn, decoder_rnn, W_p_gen, bias_p_gen
...
def load_best_model(alphabet, results_file_path, input_dim, hidden_dim, layers, feature_alphabet,
    ...
    model, char_lookup, feat_lookup, R, bias, encoder_frnn, encoder_rrnn, decoder_rnn, W_p_gen, bias_p_gen = build_model(alphabet, input_dim, hidden_dim,
    ...
    for e in xrange(epochs):
        ...
        for i, example in enumerate(train_set):
            ...
            loss = one_word_loss(model, char_lookup, feat_lookup, R, bias, encoder_frnn, encoder_rrnn, decoder_rnn, lemma, feats, word,
                                 alphabet_index, alignment, feat_index, feature_types, W_p_gen, bias_p_gen)
            ...
            if len(dev_lemmas) > 0:
                ...
                for i in xrange(len(dev_lemmas)):
                    total_dev_loss += one_word_loss(model, char_lookup, feat_lookup, R, bias, encoder_frnn, encoder_rrnn, decoder_rnn, dev_lemmas[i],
                                                    ...
                                                    dev_aligned_pairs[i], feat_index, feature_types, W_p_gen, bias_p_gen).value()
...
def one_word_loss(model, char_lookup, feat_lookup, R, bias, encoder_frnn, encoder_rrnn, decoder_rnn, lemma, feats, word, alphabet_index, aligned_pair,
                  feat_index, feature_types, W_p_gen, bias_p_gen):
    pc.renew_cg()
    ...
    W_p_gen = pc.parameter(W_p_gen)
    bias_p_gen = pc.parameter(bias_p_gen)
    ...
    for align_index, (input_char, output_char) in enumerate(zip(aligned_lemma, aligned_word)):
        ...
        if output_char == END_WORD:
            ...
            sigmoid_input = pc.concatenate([prev_output_vec, decoder_rnn_output, blstm_outputs[i], feats_input])
            p_gen = pc.logistic(W_p_gen * sigmoid_input + bias_p_gen)
                                   
            probs_copy_np = np.zeros(len(alphabet_index))
            try:
                probs_copy_np[alphabet_index[padded_lemma[i]]] = 1
            except KeyError:
                probs_copy_np[alphabet_index[UNK]] = 1
...
#            print len(probs_copy.vec_value())
#            print len(probs_gen.vec_value())
            probs = pc.cmult(p_gen, probs_gen) + pc.cmult((1-p_gen), probs_copy)
...
def predict_output_sequence_nbest(model, char_lookup, feat_lookup, R, bias, encoder_frnn, encoder_rrnn, decoder_rnn, lemma, feats, alphabet_index,
                                  inverse_alphabet_index, feat_index, feature_types, W_p_gen, bias_p_gen, nbest):
    pc.renew_cg()
...
def predict_output_sequence_beam(model, char_lookup, feat_lookup, R, bias, encoder_frnn, encoder_rrnn, decoder_rnn, lemma, feats, alphabet_index,
                                 inverse_alphabet_index, feat_index, feature_types, W_p_gen, bias_p_gen, nbest=0):
    pc.renew_cg()
...
def predict_output_sequence(model, char_lookup, feat_lookup, R, bias, encoder_frnn, encoder_rrnn, decoder_rnn, lemma, feats, alphabet_index,
    ...
    while num_outputs < MAX_PREDICTION_LEN * 3:
        ...
        probs_copy_np = np.zeros(len(alphabet_index))
#        print padded_lemma
#        print padded_lemma[i]
#        print alphabet_index[padded_lemma[i]]
        try:
            probs_copy_np[alphabet_index[padded_lemma[i]]] = 1
            probs_copy = pc.inputTensor(probs_copy_np)
            probs = pc.cmult(p_gen, probs_gen) + pc.cmult((1-p_gen), probs_copy)
            ...
            predicted_output = padded_lemma[i]
            predicted_output_sequence.append(predicted_output)
...
def predict_sequences(model, char_lookup, feat_lookup, R, bias, encoder_frnn, encoder_rrnn, decoder_rnn, alphabet_index, inverse_alphabet_index, lemmas,
                      feats, feat_index, feature_types, W_p_gen, bias_p_gen, beam=False, nbest=0):
    ...
    for i, (lemma, feat_dict) in enumerate(zip(lemmas, feats)):
        predicted_sequence = predict_output_sequence_template(model, char_lookup, feat_lookup, R, bias, encoder_frnn, encoder_rrnn, decoder_rnn, lemma,
                                                     ...
                                                     feature_types, W_p_gen, bias_p_gen, nbest)
...
def evaluate_ndst(alphabet, alphabet_index, ensemble, feat_index, feat_input_dim, feature_alphabet, feature_types,
    ...
    if ensemble:
        ...
        for ens in ensemble_model_names:
            model, char_lookup, feat_lookup, R, bias, encoder_frnn, encoder_rrnn, decoder_rnn, W_p_gen, bias_p_gen = load_best_model(
                alphabet,
                ens,
                input_dim,
                hidden_dim,
            ...
            ensemble_models.append((model, char_lookup, feat_lookup, R, bias, encoder_frnn, encoder_rrnn, decoder_rnn, W_p_gen, bias_p_gen))
        ...
        for em in ensemble_models:
            count += 1
            model, char_lookup, feat_lookup, R, bias, encoder_frnn, encoder_rrnn, decoder_rnn, W_p_gen, bias_p_gen = em
        ...
        best_model, char_lookup, feat_lookup, R, bias, encoder_frnn, encoder_rrnn, decoder_rnn, W_p_gen, bias_p_gen = load_best_model(alphabet,
        ...
        try:
            predicted_sequences = predict_sequences(best_model,
                                                    ...
                                                    test_feat_dicts,
                                                    feat_index,
                                                    feature_types, W_p_gen, bias_p_gen, beam, nbest)
