...
if __name__ == "__main__":
    ...
    parser.add_argument("--lang", help="language", required=True, type=str)
    ...
    if args.task == 1:
        print "{0}\t{1:.1f}\t{2:.2f}".format(args.lang,*eval_form(D_gold, D_guess))
...
#        print "accuracy:\t{0:.2f}\nlevenshtein:\t{1:.2f}\nparadigm:\t{2:.2f}".format(*eval_form(D_gold, D_guess, ignore=ignore)+(eval_paradigm(D_gold, D_guess),))
        print "{0}\t{0:.2f}\t{1:.2f}".format(args.lang,*eval_form(D_gold, D_guess, ignore=ignore)+(eval_paradigm(D_gold, D_guess),))
